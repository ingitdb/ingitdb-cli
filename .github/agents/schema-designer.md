...
description: 'Use this agent to design inGitDB schemas: .ingitdb.yaml (root config),
  .ingitdb-collection.yaml (collection schemas), and .ingitdb-view.<name>.yaml (materialized
  view definitions). Given a description of data to store, schema-designer produces
  ready-to-use YAML files with explanatory comments. Also use it to review and improve
  an existing schema, or to check whether a schema is valid before running the validator.
