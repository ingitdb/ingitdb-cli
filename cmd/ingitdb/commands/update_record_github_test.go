...
func TestUpdateRecord_GitHub_ParseError(t *testing.T) {
	...
	cmd := updateRecord(homeDir, getWd, readDefinition, newDB, func(...any) {})
	err := runCobraCommand(cmd, "--id=test.items/x", "--set={name: X}", "--github=invalid")
	if err == nil {
...
func TestUpdateRecord_GitHub_ReadDefinitionError(t *testing.T) {
	...
	cmd := updateRecord(homeDir, getWd, readDefinition, newDB, func(...any) {})
	err := runCobraCommand(cmd, "--id=test.items/x", "--set={name: X}", "--github=owner/repo")
	if err == nil {
...
func TestUpdateRecord_GitHub_DBOpenError(t *testing.T) {
	...
	mockReader := &fakeFileReader{files: map[string][]byte{
		...
		"test-ingitdb/items/.collection/definition.yaml": []byte("record_file:\n  name: items.json\n  type: map[string]map[string]any\n  format: json\ncolumns:\n  name:\n    type: string\n"),
	}}
...
func TestUpdateRecord_GitHub_CollectionNotFound(t *testing.T) {
	...
	mockReader := &fakeFileReader{files: map[string][]byte{
		".ingitdb.yaml": []byte("rootCollections:\n  other.items: test-ingitdb/items\n"),
		"test-ingitdb/items/.collection/definition.yaml": []byte("record_file:\n  name: items.json\n  type: map[string]map[string]any\n  format: json\ncolumns:\n  name:\n    type: string\n"),
	}}
	mockReaderFactory := NewMockGitHubFileReaderFactory(ctrl)
	mockReaderFactory.EXPECT().NewGitHubFileReader(gomock.Any()).Return(mockReader, nil).AnyTimes()
	...
	mockDBFactory := NewMockGitHubDBFactory(ctrl)
	mockDBFactory.EXPECT().NewGitHubDBWithDef(gomock.Any(), gomock.Any()).Return(nil, errors.New("unused - shouldn't be called")).AnyTimes()
