...
type demoNextStep struct {
	...
	// Shell, when set, is the only shell Command is quoted for ("cmd.exe" or
	// "PowerShell"); the step then has one entry per shell.
...
func demoNextSteps(dir, goos string) []demoNextStep {
	...
	ingitdbSteps := []demoNextStep{
		{Step: 1, Label: "Browse the lists in the terminal UI", Command: "ingitdb"},
		{Step: 2, Label: "Query the lists", Command: "ingitdb select --from=" + collection},
		{Step: 3, Label: "Query what to buy", Command: "ingitdb select --from=" + firstList + "/" + demoItemsCollection},
	}
	var steps []demoNextStep
	for _, step := range ingitdbSteps {
		for _, q := range demoPathQuotings(dir, goos) {
			withPath := step
			withPath.Shell = q.shell
			withPath.Command = step.Command + " --path=" + q.path
			steps = append(steps, withPath)
		}
	}
	return append(steps,
		demoNextStep{Step: 4, Label: demoOVDBLabel, Command: "ovdb demo install --yes"},
		demoNextStep{Step: 4, Label: demoOVDBLabel, Command: "ovdb demo open", Note: demoOVDBNote},
	)
}

type demoQuotedPath struct{ shell, path string }

// demoPathQuotings returns dir quoted for pasting into the shells of goos.
// On Windows one double-quoted path works in both cmd.exe and PowerShell,
// unless it holds characters those shells expand even inside double quotes
// (% for cmd.exe, $ and ` for PowerShell): then each shell gets its own.
func demoPathQuotings(dir, goos string) []demoQuotedPath {
	if goos != "windows" || !strings.ContainsAny(dir, "%$`") {
		return []demoQuotedPath{{path: demoShellQuote(dir, goos)}}
	}
	powerShell := strings.NewReplacer("`", "``", "$", "`$").Replace(dir)
	return []demoQuotedPath{
		// ^% outside the quotes is a literal % for cmd.exe.
		{shell: "cmd.exe", path: `"` + strings.ReplaceAll(dir, "%", `"^%"`) + `"`},
		{shell: "PowerShell", path: `"` + powerShell + `"`},
	}
}

// demoShellQuote quotes s for pasting into the shell of goos when it holds
// anything but plain path characters: double quotes on Windows, POSIX single
// quotes elsewhere.
