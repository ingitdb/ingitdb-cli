...
// TestRebase_ReadmeOnlyConflict_ContinueFails creates a git repo with a
// README-only conflict, resolves it via readDefinition side-effect (stages
// README.md), so git commit succeeds. Then git rebase --continue runs —
// if it fails this covers rebase.go L105 (contErr return).
func TestRebase_ReadmeOnlyConflict_ContinueFails(t *testing.T) {
	dir := t.TempDir()
	runGit(t, dir, "init")
	runGit(t, dir, "config", "user.email", "test@example.com")
	runGit(t, dir, "config", "user.name", "Test User")
	...
	readmePath := filepath.Join(dir, "README.md")
	dummyPath := filepath.Join(dir, "dummy.txt")
	if err := os.WriteFile(readmePath, []byte("# Original\n"), 0o644); err != nil {
		t.Fatalf("WriteFile: %v", err)
	}
	if err := os.WriteFile(dummyPath, []byte("x"), 0o644); err != nil {
		t.Fatalf("WriteFile: %v", err)
	...
	// main: TWO commits that change README.md (so rebase --continue hits a second conflict)
	if err := os.WriteFile(readmePath, []byte("# First change on main\n"), 0o644); err != nil {
		t.Fatalf("WriteFile: %v", err)
	}
	runGit(t, dir, "add", ".")
	runGit(t, dir, "commit", "-m", "main readme change 1")

	if err := os.WriteFile(readmePath, []byte("# Second change on main\n"), 0o644); err != nil {
		t.Fatalf("WriteFile: %v", err)
	}
	runGit(t, dir, "add", ".")
	runGit(t, dir, "commit", "-m", "main readme change 2")

	// base: conflicting README.md change
	runGit(t, dir, "checkout", "base")
	...
	getWd := func() (string, error) { return dir, nil }
	readDef := func(wd string, _ ...ingitdb.ReadOption) (*ingitdb.Definition, error) {
		// Side effect: resolve the README conflict by accepting ours and staging
		rp := filepath.Join(wd, "README.md")
		_ = os.WriteFile(rp, []byte("# Resolved by test\n"), 0o644)
		_ = exec.Command("git", "-C", wd, "add", "README.md").Run()
		return &ingitdb.Definition{}, nil
	}
	logf := func(...any) {}
	...
	cmd := Rebase(getWd, readDef, logf)
	err := runCobraCommand(cmd, "--base_ref=base")
	// First conflict resolved, commit succeeds. Then rebase --continue
	// tries to apply the second commit which also conflicts → contErr.
	if err != nil {
		t.Logf("rebase result (expected — second conflict): %v", err)
	}
	_ = runGitNoFail(dir, "rebase", "--abort")
}

// TestRebase_ReadmeOnlyConflict_CommitFails creates a git repo where only
// README.md is in conflict. With an empty definition, docsUpdate does
// nothing, so git commit --no-verify fails (nothing staged, conflicts
// remain). This covers the commitErr return path (rebase.go L102).
func TestRebase_ReadmeOnlyConflict_CommitFails(t *testing.T) {
	dir := t.TempDir()
	runGit(t, dir, "init")
	runGit(t, dir, "config", "user.email", "test@example.com")
	runGit(t, dir, "config", "user.name", "Test User")

	// Create initial commit with a README.md and a dummy file to avoid empty commits.
	readmePath := filepath.Join(dir, "README.md")
	dummyPath := filepath.Join(dir, "dummy.txt")
	if err := os.WriteFile(readmePath, []byte("# Original\n"), 0o644); err != nil {
	...
	// Back to main → rebase onto base creates README-only conflict
	runGit(t, dir, "checkout", "main")

	getWd := func() (string, error) { return dir, nil }
	readDef := func(_ string, _ ...ingitdb.ReadOption) (*ingitdb.Definition, error) {
		return &ingitdb.Definition{}, nil // empty def → docsUpdate does nothing
	}
	logf := func(...any) {}

	cmd := Rebase(getWd, readDef, logf)
	err := runCobraCommand(cmd, "--base_ref=base")
	// Expected: rebase creates README conflict → enters resolve path →
	// docsUpdate does nothing → git commit fails (nothing staged).
	// This covers the commitErr return path.
	if err != nil {
		t.Logf("rebase error (expected): %v", err)
	}
	// Clean up in-progress rebase
	_ = runGitNoFail(dir, "rebase", "--abort")
}
