...
func (m homeModel) Update(msg tea.Msg) (homeModel, tea.Cmd) {
	...
	case tea.KeyMsg:
		...
		case "down", "j":
			max := len(m.collections) // last position = add button
			if m.cursor < max {
				m.cursor++
			}
		}
...
func (m homeModel) View() string {
	...
	leftInner := leftWidth - 4   // subtract border + padding
	rightInner := rightWidth - 4 // subtract border + padding
	innerH := m.height - 5       // header(1) + help(1) + borders(2) + padding
...
func (m homeModel) renderCollectionList(width, height int) string {
	...
	maxVisible := height - 3 // title + add button
	start := 0
...
func (m homeModel) renderWelcome(width, _ int) string {
	...
	title := titleStyle.Render("Welcome to inGitDB CLI")
	sb.WriteString(title)
	sb.WriteString("\n\n")
...
// wordWrap wraps text at word boundaries within the given width.
...
// shortenPath truncates a path to fit within maxLen by replacing the middle
// with "…".
