...
### [CODE] 3.1 Add notarize block to `.github/goreleaser-homebrew.yaml` ⏱ ~5 min

Add the following block between `checksum` and `release` in `.github/goreleaser-homebrew.yaml`:
...
The `enabled: '{{ isEnvSet "MACOS_SIGN_P12" }}'` guard in goreleaser-homebrew.yaml means signing is
